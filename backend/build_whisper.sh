...
log_info "Note: Ensure Visual Studio Build Tools and CMake are installed"
...
# Use MSVC compiler
cmake -G "Visual Studio 17 2022" -A x64 .. || handle_error "CMake configuration failed"
cmake --build . --config Release || handle_error "Build failed"
cd ..
...
SERVER_BINARY=""
if [ -f "build/bin/Release/whisper-server.exe" ]; then
    SERVER_BINARY="build/bin/Release/whisper-server.exe"
elif [ -f "build/Release/whisper-server.exe" ]; then
    SERVER_BINARY="build/Release/whisper-server.exe"
elif [ -f "build/examples/server/Release/server.exe" ]; then
    SERVER_BINARY="build/examples/server/Release/server.exe"
else
    handle_error "Could not find whisper-server executable."
fi

log_info "Found server binary at: $SERVER_BINARY"
cp "$SERVER_BINARY" "$PACKAGE_NAME/whisper-server.exe" || handle_error "Failed to copy server binary"
...
if [ ! -d "venv" ]; then
    ...
    source venv/Scripts/activate || handle_error "Failed to activate virtual environment (Windows)"
else
    source venv/bin/activate || handle_error "Failed to activate virtual environment (Unix)"
fi
